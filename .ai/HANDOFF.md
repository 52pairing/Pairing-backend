...
### 레포
전 레포 develop 최신 · 미커밋 0. backend `41ec697`, python `6eedb6d`, frontend `a28ee79`.

### 이번에 머지된 내 작업 3건
- `62384a3` 후보 프로필 스냅샷 + 협상 출발 조건 + AI 재시도 + 재추천 429 해결
- `06beba5` 프로젝트 취소 시 응답 대기 요청 종결 (3번 요청, P46)
- `c08ce7d`·`e0e9e9f` 요청 상세 프로젝트 본문 7개 + **후보 카드 조건 동결**

마지막 건이 성능에도 영향이 있다 — 후보 1명당 2쿼리씩 돌던 `findCondition` 이 사라지고
포지션 스냅샷을 한 번에 읽는다. **후보 10명 기준 20쿼리 → 1쿼리.**

**662 tests, 0 failures.**

### 다음 작업 — 성능 최적화 (아직 착수 안 함)

| 순위 | 항목 | 예상 |
|---|---|---|
| 1 | 매칭 요청 목록 N+1 (`MatchingRequestResponseAssembler`) | 20건 약 140쿼리 → 6~8 |
| 2 | 비동기 스레드풀 — 매칭 전용 풀 분리 | 동시 5 → 20 |
| 3 | 후보 목록 남은 N+1 (`findCardSummary`·`existsByCandidateId`) | 20 → 2 |
| 4 | 추천 시 스냅샷 루프 (1명당 5쿼리) | 절반 |
| 5 | `(position_id, is_exposed)` 인덱스 | EXPLAIN만 |
| 6 | 캐시 (`@EnableCaching` 이 레포 전체에 없음) | 1~3 뒤 |

**측정 전에 반드시** — `org.hibernate.SQL` 을 `warn` 으로 내리고, 1 vCPU 로 묶어 띄운다.
로그를 켠 채 k6 를 돌리면 로그 I/O 가 병목이 되어 숫자가 전부 왜곡된다.

**AI 경로 부하는 파이썬 스텁으로** — `AI_STUB_MODE=true` / `AI_STUB_DELAY_MS=20000` / `JITTER 3000`.
실제 호출로 걸면 "부하가 늘 때 응답이 어떻게 변하나" 대신 "쿼터가 언제 떨어지나"를 재게 된다.

### 주의 사항 (9~11)

9. **비동기 풀이 실제로는 5개다.** `AsyncConfig` 가 core 5 / max 10 / queue 500 인데,
   `ThreadPoolTaskExecutor` 는 큐가 꽉 차야 스레드를 늘린다. 큐가 500이라 영원히 5개다.
   AI 호출이 20~60초라 6번째 요청부터 **에러도 타임아웃도 없이 대기**한다.
   **계약 도메인은 이미 전용 풀(2개)로 피해 뒀다**(`ContractDraftExecutorConfig`) — 같은 패턴을 따르면
   공용 설정을 안 건드린다. 크론도 `30 */5` 로 30초 밀어 매칭 회차 복구와 안 겹치게 해뒀다.
10. **pull 안 하고 "코드에 없다"고 판단하지 말 것.** 계약 AI 재시도 파일이 없다고 보고했는데
    develop 에 이미 있었다. 팀원 문서와 코드가 다르면 **먼저 fetch 부터.**
11. **매칭도 챗봇도 RAG 가 아니다.** 검색 결과를 프롬프트에 넣지 않는다 —
    매칭은 후보 목록, 챗봇은 통과 여부 판정이다. 문서·발표에서 RAG 라고 쓰면 설명이 막힌다.
