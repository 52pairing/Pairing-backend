...
- [ ] G3 추가 — 후보 희망 월단가 ≤ `budgetCap`(포지션별, 조합 아님). **탈락시키지 말 것** —
      `guardPassed=true` 유지하고 `guardReason`에 사유만 남긴다. 배제하면 Stage B 폐기 사유
      (단가로 거르면 사전검수 후보수와 어긋남)가 되살아난다
...
| # | 내용 |
|---|---|
| ~~U1~~ | **확정: 포지션별 `budgetCap` 이내만 확인(조합 아님), 탈락 없이 경고만.** 4단계 참고 |
| U2 | 프로젝트명(`title`)을 임베딩에 넣을지 |
| U3 | 유사도 정규화 상수 (재색인 후 실측) |
| U4 | 예산 경고를 화면에 띄울지 (띄우려면 회차 플래그 + API 필드 + 프론트 문서 갱신) |
