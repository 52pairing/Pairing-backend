...
- **PR #61(임베딩 재생성) merge 후 발생한 `.ai/HANDOFF.md`/`WORKLOG.md` merge 충돌 2건 해결** (`feature/project-updated-embedding-refresh` → develop 머지 시, 그다음 `feature/matching-project-stage-sync` → develop 머지 시 각각 1번씩). 둘 다 여러 브랜치가 같은 문서의 같은 줄(항목 번호 22~25, "다음 세션에서 할 일" 목록)을 독립적으로 고쳐서 난 충돌 — 코드 충돌은 없었음. 로컬에서 `git merge origin/develop`으로 재현 → 양쪽 브랜치의 고유 내용을 다 보존하는 방향으로 수동 병합(어느 한쪽만 채택하지 않음, 항목 번호 재정렬, 이미 merge된 PR엔 "PR #번호로 merge됨" 표시 추가) → `./gradlew clean build` 통과 확인 후 병합 커밋 push. GitHub PR 화면의 `mergeable: true` 전환으로 재확인.
- **매칭 pgvector 설정 문의 답변**: 매칭(Java)은 pgvector를 전혀 안 건드림 — `MatchingPort`로 Pairing-python의 REST API만 호출. 실제 설정은 Pairing-python 쪽(`db/init/10-create-ai-schema.sql`: `CREATE EXTENSION vector`, `freelancer_embedding`/`position_embedding` `vector(768)`, `freelancer_embedding`에만 ivfflat 코사인 인덱스 — 검색이 이 테이블에서만 일어나서 정상, `position_embedding`은 ID 단건 조회라 인덱스 불필요). SQLAlchemy + `pgvector.sqlalchemy.Vector`, asyncpg로 Spring과 같은 PostgreSQL에 별도 커넥션. **확인 중 발견**: `Pairing-backend/docker-compose.yml`의 postgres가 순정 `postgres:16-alpine`이라 pgvector 확장이 기본 포함 안 됨 — 로컬에서 새로 셋업하면 `CREATE EXTENSION vector`가 실패할 수 있음. README엔 Python 클라이언트 패키지(`pip install pgvector`) 설치만 있고 서버 쪽 확장 설치 방법은 문서화 안 돼 있음. 누가 이걸로 막혔다면 `pgvector/pgvector:pg16` 이미지로 교체하거나 확장을 직접 설치해야 함 — 공유 개발/스테이징 DB는 이미 누가 한 번 실행해놨을 가능성 있어 환경별로 확인 필요.
...
11. **(신규, 낮은 우선순위, 매칭 담당 아님)** `Pairing-backend/docker-compose.yml`의 postgres가 `postgres:16-alpine`이라 pgvector 확장이 기본 미포함 — 로컬 신규 셋업 시 `CREATE EXTENSION vector` 실패 가능. `pgvector/pgvector:pg16` 이미지 교체나 확장 설치 방법 문서화 필요(누가 담당할지는 인프라/3번과 상의). 매칭이 직접 건드리는 파일은 아니라 상황 공유만.
