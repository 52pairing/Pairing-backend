...
## 2026-08-13 (6) — 후보 카드 상태값을 서버가 내려주기 / 거절 취소는 안 만듦

### 결정

- **거절 취소(롤백) API는 만들지 않는다.** 사용자 확정.
- **선택·거절한 후보도 목록에 계속 나온다.** 숨기지 않고 상태를 표시한다.

숨기지 않는 이유는 두 가지다. 요청한 후보를 숨기면 클라이언트가 **누구에게 요청했는지** 볼 수 없다.
거절한 후보는 되돌리는 API가 없고 R02 예외조건 5로 다음 회차에도 안 나오므로, 화면에서까지 지우면
그 후보를 **영구히 잃는다** — (5)에서 고친 버그와 성격이 같다.

거절 취소를 안 만드는 대신 **프론트에 거절 확인 팝업**을 넣기로 했다(전달 문서에 명시).

### 넣은 것

`CandidateResponse.status`(코드) + `statusLabel`(문구). `AVAILABLE`(선택 가능) /
`REQUESTED`(요청 보냄) / `REJECTED`(거절함).

**거절이 요청보다 우선이다.** 둘 다 참일 수 있는데(요청을 보낸 뒤에도 거절 가능), 카드로 무엇을 할 수
있는지를 정하는 건 거절 쪽이다 — `MatchingCandidate.isSelectable()`이 `rejected`면 무조건 false다.
프론트가 두 불리언을 직접 조합하면 우선순위를 다르게 잡을 수 있고, 그러면 "선택 가능"으로 그려진
카드를 눌렀을 때 서버가 `MT_017`로 거부한다. **표시 기준과 서버 판정 기준이 같아야 한다.**

기존 `requested`/`rejected` 불리언은 그대로 뒀다(additive).

### 검증

`./gradlew clean build` — **542 tests, 0 failures**(신규 3건). 선택·거절 후에도 목록에 남는지,
둘 다 참일 때 `REJECTED` 가 이기는지 검증한다.

### 문서

바탕화면 `AI매칭_API_화면매핑_최신본.md` 갱신 — 맨 위 변경 요약 3건, `CandidateResponse` 필드표,
"카드 상태" 절 신규, "버튼/화면 상태 처리 권장"을 `status` 기준으로 교체.

---

## 2026-08-13 (7) — 후보 카드 상태를 셋 중 하나로 배타적이게

### 결정

**요청 보냄 / 거절함 / 아무것도 안 함 셋 중 하나여야 한다.** 사용자 확정.

(6)까지는 요청을 보낸 뒤에도 거절할 수 있어서 두 불리언이 동시에 참일 수 있었고, 그래서
"거절이 요청보다 우선"이라는 우선순위 규칙이 필요했다. 규칙 자체가 화면과 서버가 갈릴 여지였다.

### 넣은 것

`MatchingErrorCode.CANDIDATE_ALREADY_REQUESTED`(MT_019) 추가. `rejectCandidate` 진입부에서
`existsByCandidateId`로 막는다.

반대 방향은 원래부터 막혀 있었다 — `MatchingCandidate.isSelectable()`이 `rejected`면 false라
거절한 후보에게 요청을 보내면 MT_017이 난다. **양쪽이 다 막혀야 셋이 배타적이다.**

`Status.of`의 `rejected` 우선 판정은 남겼다. 이 가드 이전에 만들어진 데이터가 있을 수 있고,
그때도 화면 표시가 `isSelectable()`과 같은 답을 내야 한다.

### 왜 요청 쪽을 못 되돌리게 했나

이미 보낸 요청은 프리랜서가 응답을 기다리는 중이라, 클라이언트가 후보 카드를 내려도 그 요청이
없어지지 않는다. 카드만 "거절함"이 되고 요청은 살아 있으면 화면이 거짓말을 한다.

### 검증

`./gradlew clean build` — **543 tests, 0 failures**. 회귀 테스트 2건(양방향 각 1건).

**변이 테스트**: 가드를 무력화하면 거절이 200으로 통과해 실패한다(`expected:<400> but was:<200>`).

### 문서

바탕화면 `AI매칭_API_화면매핑_최신본.md` — 변경 요약, "카드 상태" 절(배타성 표), "2. 추천 후보 거절"
절에 MT_019와 **거절 확인 팝업 요청**, 에러코드 표에 MT_019 추가.

---

## 2026-08-13 (8) — 결제 직후 추천 후보 탭이 404로 뜨던 문제

### 증상

착수금 결제 후 「추천 후보」 탭에 들어가면 **"추천 라운드를 찾을 수 없습니다"**(MT_001, 404) 가 뜨고
"추천 후보를 불러오지 못했습니다" 화면이 나온다. **"다시 시도"를 누르면 그제서야 후보가 보인다.**

### 원인

최초 추천은 `RecruitingStartedEventListener`(`@Async`)가 모집 시작 이벤트를 받아 만든다. LLM 호출까지
포함해 **수 초~수십 초**가 걸린다. 그 사이에는 라운드가 없다.

`findCandidates`가 그때 `ROUND_NOT_FOUND`를 던지고 있었다. **정상적인 대기 상태를 에러로 내보낸 것**이라,
프론트는 에러 화면을 그릴 수밖에 없었다.

### 고친 것

라운드가 없으면 200 + `preparing: true`로 내려간다(`CandidateListResponse.preparing`).
`candidates`는 빈 배열, `roundId`는 null, **`headcount`는 채운다**(대기 중에도 "0/4명"을 그려야 한다).

소유자 확인은 그대로 한다. 평소엔 라운드에서 projectId를 얻는데 라운드가 없으므로,
`ProjectDirectoryPort.findPositionSummary(positionId)` 단일 인자 버전을 추가해 포지션에서 프로젝트를
거슬러 올라간다(어댑터는 기존 2인자 메서드를 재사용해서 조립이 두 벌이 되지 않게 했다).

### 테스트를 두 번 짠 이유 (기록해둠)

소유자 검증 테스트를 처음엔 **프리랜서 토큰**으로 짰다. 통과했지만 **아무것도 검증하지 못했다** —
컨트롤러에 `@PreAuthorize("hasRole('CLIENT')")`가 있어서 내 검사에 닿기도 전에 403이 난다.
변이 테스트(검사 제거)에서 여전히 통과하는 것을 보고 발견했다.

**프로젝트를 소유하지 않은 두 번째 클라이언트**로 다시 짜니 변이에서 `expected:<403> but was:<200>`
으로 실패한다. 권한 테스트는 **막히는 이유가 내가 의도한 그 검사인지**까지 확인해야 한다.

### 검증

`./gradlew clean build` — **545 tests, 0 failures**. 회귀 테스트 2건.

### 남는 것

추천 생성이 아예 실패하면(AI 서버 장애 등) `preparing: true`가 계속 내려간다. 서버가 "실패"를 알 방법이
없다 — 리스너가 죽으면 라운드 자체가 안 생긴다. 프론트에 **폴링 2~3분 상한**을 요청했고,
관리자 진단의 `ROUND_MISSING`이 이 상태를 잡는다.

---

## 2026-08-13 (9) — "추천 준비중"에서 영영 안 끝나는 경우 없애기

(8)에서 대기 상태를 `preparing`으로 내리게 했는데, **추천 생성이 아예 실패하면 그 화면에서 영원히
멈추는** 문제가 남아 있었다. 그걸 없앴다.

### 진짜 원인은 트랜잭션 경계였다

`RecruitingStartedPositionHandler.startInitialRecommendation`이 `@Transactional(REQUIRES_NEW)` **하나로**
스냅샷 + 회차 생성 + **AI 호출**까지 묶고 있었다. 그래서:

- AI 호출이 끝날 때까지 **회차 행이 커밋되지 않는다** → 그 수십 초 동안 라운드가 아예 없다
- AI 호출이 실패하면 **회차 행까지 롤백돼 사라진다** → 무엇이 실패했는지 아무 데도 안 남는다
- `round.fail()`은 재추천 경로에만 있었다. 최초 추천은 실패를 기록할 자리 자체가 없었다

**재추천 경로는 원래부터 쪼개져 있었다**(`openRound` 동기 + `RerecommendRoundFiller` 별도 트랜잭션).
최초 추천만 안 그랬다.

### 고친 것

1. **트랜잭션을 쪼갰다.** `openInitialRound`(스냅샷+회차, 즉시 커밋) → `MatchingRoundFiller.fill`
   (AI 호출, 별도 트랜잭션) → 실패하면 `markFailed`. `RerecommendRoundFiller`를 `MatchingRoundFiller`로
   이름만 바꿔 두 경로가 같이 쓴다.
2. **조회가 회차 상태를 본다.** 회차가 생기자마자 커밋되므로 "회차 있음 = 후보 있음"이 더는 성립하지
   않는다. `RUNNING → preparing`, `FAILED → failed`. 안 그러면 채우는 중인 회차가 **"후보 없음"으로**
   보인다.
3. **멈춘 회차 복구 스케줄러**(`StaleRoundRecoveryService`, 5분 주기). AI 호출 중 컨테이너가 교체되면
   스레드가 흔적 없이 사라지고 회차가 RUNNING으로 남는다 — 오늘만 태스크 정의가 네 번 바뀌었다.
   5분 넘게 RUNNING이면 다시 채우고, 그것도 실패하면 FAILED로 닫는다.

### 왜 재시도가 안전한가

후보 저장과 회차 완료가 한 트랜잭션이라, 중간에 죽은 회차에는 **후보가 하나도 없다**. 다시 채워도
중복이 안 생긴다. 반대로 후보가 저장됐다면 회차는 이미 COMPLETED라 복구 대상에 안 걸린다.

### 왜 FAILED로 닫지 않고 재시도하나

최초 추천이 실패하면 클라이언트에게 복구 수단이 없다 — 무료 재추천은 "보낸 요청 전원 거절"일 때만
열리고(P41), 유료 재추천은 돈을 낸다. **착수금을 이미 낸 클라이언트에게 다시 결제를 요구할 수는 없다.**

### 검증

`./gradlew clean build` — **550 tests, 0 failures**. 회귀 테스트 5건.
...
## 2026-08-14 (2) — 결제 순간 AI 서버가 죽어 있으면 "준비중"에서 영원히 멈추던 구멍

### 어제 고친 게 절반만 적용돼 있었다

어제 "AI 호출은 회차가 커밋된 뒤에" 원칙을 세웠는데, `openInitialRound` 안에 **AI 호출이 하나 더
남아 있었다** — 포지션 임베딩 생성(`upsertPositionEmbedding`)이 회차를 만들기 **전에** 있었다.

```
스냅샷 → [AI] 임베딩 → 회차 커밋 → [AI] 추천
           ↑ 여기서 실패하면 트랜잭션 롤백 → 회차도 스냅샷도 안 남음
```

회차가 없으면 후보 조회는 `preparing: true`(준비중)를 내려주고, 복구 스케줄러는 **RUNNING 회차만**
찾으므로 아무것도 못 한다. **화면이 영원히 "준비중"에서 멈춘다.** 착수금을 이미 낸 클라이언트에게
복구 수단이 없다.

파이썬이 죽어 있으면 두 AI 호출이 다 실패하므로, **결제 시점에 AI 서버가 내려가 있으면 이 경로로
간다.** 오늘 파이썬 배포가 여러 번 있었으니 드문 상황이 아니다.

### 고친 것

포지션 임베딩 생성을 `fillCandidates`(2단계)로 옮겼다. 회차가 커밋된 뒤에 부르므로, 실패해도
회차가 `RUNNING`/`FAILED` 로 남아 복구 스케줄러가 잡아간다.

**최초 추천에서만 만든다**(`round.getRoundType() == INITIAL`). 재추천은 이미 있는 벡터를 쓴다 —
매번 다시 부르면 같은 값으로 덮어쓰면서 AI 호출만 늘어난다. 복구가 최초 추천 회차를 다시 채울
때는 이 경로를 다시 타는데, upsert 라 중복이 생기지 않는다.

핸들러에서 `MatchingPort` 의존이 필요 없어져 제거했다.

### 검증

`./gradlew clean build` — **611 tests, 0 failures**.

**변이 테스트**: 임베딩 호출을 회차 생성 트랜잭션 안으로 되돌리면 신규 테스트가
`NoSuchElementException: No value present` 로 실패한다 — 회차 자체가 없다는 뜻이다.

---

## 2026-08-14 (3) — 후보 프로필 스냅샷, 협상 출발 조건, 재추천 레이트리밋 제거

### 1) 후보 프로필 스냅샷 (노출 시점)

화면 요구: "추천 후보에 뜨기 전에 작성한 프로필"을 상세에서 보여줄 것. 프리랜서가 추천된 뒤 이력서를
고쳐도 클라이언트가 본 내용이 바뀌면 안 된다.

`MatchingRoundCreationService.saveFreelancerSnapshots` — **노출이 확정된 후보만** card + condition +
resume + capturedAt 을 `matching_snapshot`(FREELANCER)에 얼린다. 대기 순번은 화면에 안 뜨므로 제외한다.
같은 (freelancerId, positionId)에 이미 있으면 건너뛴다(재추천 때 중복 저장 방지).

조회는 `GET /api/v1/matchings/candidates/{candidateId}/profile`.

**한 명이 실패해도 추천은 계속한다.** `findResume`/`findCondition`은 자료가 없으면 MT_015를 던지는데,
그게 캡처 도중 터지면 같은 트랜잭션이라 **방금 저장한 후보 전체가 롤백된다** — 후보 4명 중 1명이
이력서를 지웠으면 4명 다 사라지고 회차가 FAILED 된다. 스냅샷은 상세 화면용 부가 자료이므로 매칭을
막아선 안 된다. 실제로 이 구조에서 통합테스트 7건이 깨져 있었고(픽스처에 이력서가 없어서),
try/catch 한 곳으로 전부 복구됐다.

**스냅샷이 없으면 현재 프로필로 대체한다.** 이 기능 배포 전에 이미 노출된 후보는 스냅샷이 없어서
상세가 아예 안 열린다. 조건·이력서는 개별적으로 감싸 없으면 null 로 둔다 — 폴백이 같은
`findResume`을 다시 부르므로, 감싸지 않으면 **캡처가 실패한 후보는 프로필이 영영 안 열린다**.
프론트 타입이 두 필드를 nullable 로 받는다.

### 2) 협상 출발 조건을 노출 시점 값으로

수락 시점에 찍던 FREELANCER 스냅샷을 **없앴다**. 두 시점이 다 남으면 같은 타입 안에 서로 다른 모양의
JSON이 섞이고(수락 시점은 조건 8개 평면, 노출 시점은 카드·조건·이력서 중첩) 읽는 쪽이 모양을 짐작해야
한다. `buildFreelancerSnapshot`/`writeJson`도 함께 제거했다(호출자 없음).

**협상도 노출 시점 조건에서 시작한다.** 예전엔 수락 시점의 라이브 조건을 `CreateNegotiationCommand`에
넘겼다. 클라이언트는 노출 당시 금액을 보고 후보를 골랐으므로, 추천된 뒤 프리랜서가 단가를 올렸다고
협상 출발점이 따라 올라가면 **클라이언트가 본 적도 동의한 적도 없는 숫자**에서 협상이 시작된다.
스냅샷이 없으면 라이브로 대체하고 그 사실을 로그에 남긴다
(`java.request.accept.condition_source source=LIVE`).

### 3) 재추천 레이트리밋 제거

배포 환경에서 429(MT_011)가 났다. `Retry-After: 3294`(≈55분) 이라 버스트가 아니라 **시간 한도**였다.

키가 `rate_limit:matching-rerecommend:ACCOUNT_{id}` 로 **계정 단위이고 프로젝트를 구분하지 않았다**.
한도는 5초 1회 + **1시간 10회**. 여러 프로젝트를 동시에 진행하면 정상 사용 중에 막힌다.
`Refill.intervally`라 다 쓰면 다음 채움까지 통째로 기다린다.

총량은 이미 비즈니스 규칙이 막는다(무료 1회 / 유료 5회, **프로젝트 단위** + 남은 자리 이내).
레이트리밋은 그 위에 얹힌 중복 방어였다. 인터셉터·정책·WebConfig 3개 파일을 지웠다.
전역 인프라(`RateLimitProvider`, `RedisRateLimitConfig`)는 다른 도메인이 쓸 수 있어 남겼고,
정책 빈이 없어도 기동에 문제 없음을 확인했다. `MT_011` 열거값은 프론트가 이미 다루므로 남기고
"지금은 아무도 던지지 않는다"를 주석으로 적었다.

**연타 방어가 없어졌다.** 버튼을 빠르게 두 번 누르면 회차가 2개 생기고 유료 재추천 2회가 소진된다.
프론트가 202 응답 후 완료 알림까지 버튼을 잠가야 한다(전달 문서에 명시).

### 검증

`./gradlew clean build` — **613 tests, 0 failures**.

**변이 테스트**
- 협상 출발 조건을 라이브로 되돌리면 신규 테스트가 실패한다(노출 시점 값이 안 쓰임)
- 스냅샷 캡처를 회차 생성 트랜잭션 안으로 되돌리면 회차 자체가 사라진다

**직렬화 왕복을 따로 검증했다.** 앞의 협상 테스트는 payload를 손으로 만들어 심으므로 **실제 저장
경로가 만든 JSON이 다시 읽히는지**는 증명하지 못한다. 세 record가 왕복하지 못하면 조회가 조용히
null 을 돌려주고 협상이 라이브로 새어나간다 — 에러도 안 난다. 실제 자료로 저장→조회해서
`payAmount`·`payUnit`·`workStyle`·`workForm`·카드 이름·`capturedAt` 이 살아남는 것을 확인했다.
...
## 2026-08-15 — 요청 상세에 프로젝트 본문 노출 (프론트 요청)

프리랜서가 받은 매칭 요청에서 프로젝트 상세를 볼 때 내용이 부족했다. 수락하면 곧바로 협상이
시작되는데, 뭘 하는 프로젝트인지 다 못 보고 결정해야 했다.

### 넣은 것 7개

진행 상황 · 시작일 협의 가능 여부 · 예상 기간(숫자+단위) · 전체 모집 인원 ·
세부 업무 범위 · 우대사항 · 근무 장소

`GET /api/v1/matchings/requests/{requestId}` 에만 채운다. **목록에는 안 넣었다** — 옛 요청이
섞인 20건 페이지마다 프로젝트를 20번 더 읽게 된다(아래 폴백 참고). `mainTask` 와 같은 규칙이다.

### 뺀 것 (요청 범위 밖으로 정리)

- **첨부 자료** — 다운로드가 `hasRole('CLIENT')` 전용이라 project 도메인 결정이 필요하다
- **남은 자리(confirmedHeadcount)** — 경쟁 상황 노출
- **모집 마감일** — 이번 목록에서 빠졌다

**클라이언트 내부 정보는 애초에 담지 않았다.** 결제·정산 상태, 재추천 사용 횟수, 모집 연장 횟수,
`statusNote`("2/3명 협상중")가 프로젝트 상세 조회에는 있다. 그대로 넘겼으면 프리랜서에게
"당신은 3번째 재추천으로 뽑혔다"가 드러났다.

### 조회 경로 — 요약본을 건드리지 않았다

7개 중 **근무 장소·시작일 협의 여부**만 매칭의 `ProjectPositionSummary` 에 없었다. 거기에 두 필드를
더하면 `findPositionSummary` 가 프로젝트를 한 번 더 읽어야 하는데, 그 메서드는 재색인 루프
(`EmbeddingReindexService`)와 수락·재추천·회차 생성이 전부 쓰는 경로다. 화면 표시용 필드 때문에
그쪽 전부에 조회를 붙일 이유가 없다.

그래서 `ProjectDirectoryPort.findProjectContent(projectId)` 를 따로 만들었다. 모집 시작(스냅샷
얼리기)과 요청 상세 조회에서만 부른다. `getDetail` 이 아니라 `getById` 를 쓴다 — 그쪽은 첨부 자료와
정산 ID까지 읽는데 둘 다 안 내보내는 값이다.

### 스냅샷 우선, 없으면 현재 값

기존 7개 필드와 같은 이유로 얼린다(R32) — 요청을 보낸 뒤 클라이언트가 업무 범위나 근무 장소를
바꿔도 프리랜서가 보고 판단한 내용은 그대로여야 한다.

**폴백이 꼭 필요하다.** `saveSnapshotIfAbsent` 는 이미 있는 스냅샷을 덮지 않는다. 그래서 이 필드를
담기 시작해도 **이미 모집 중인 프로젝트는 영원히 안 나온다.** 배포 시점에 진행 중이던 요청이 전부
빈 화면을 받는 것보다 현재 값이라도 보여주는 편이 낫다.

옛 스냅샷 판별은 `periodValue == null` 로 한다. 등록 시 필수라 DB가 NOT NULL이고 새 스냅샷엔 반드시
값이 있다. 선택 입력인 `detailScope` 로 판별하면 "클라이언트가 안 적은 새 스냅샷"과 구분이 안 된다.

### 세 개는 비어 있을 수 있다

`detailScope`/`extraNote`/`workLocation` 은 **프로젝트 등록 시 선택 입력**이다(`@NotBlank` 없음,
DB nullable). 필수인 것은 `mainTask` 이고 그건 이미 나가고 있었다. 프론트에 빈 값 처리를 전달했다.

`recruitDeadline` 은 모집 시작 후에만 생기지만 이번 목록에서 빠져서 해당 없음.

### 검증

`./gradlew cleanTest test` — **659 tests, 0 failures**. 신규 4건.

**변이 테스트 2건, 둘 다 잡혔다.**
- 얼린 값 대신 항상 현재 값을 읽게 하면 → 1건 실패 (스냅샷을 안 읽음)
- 폴백을 없애면 → 2건 실패 (옛 스냅샷이 전부 null)

빈 값 테스트는 선택 입력 3개를 NULL로 만들어 상세 조회가 안 깨지는지 따로 확인했다.

### 후속 — 후보 카드도 얼린 조건을 읽는다 (같은 브랜치)

위 작업 중에 화면끼리 숫자가 어긋나는 것을 발견했다.

```
1. 후보 노출 → 단가 650만원으로 얼림
2. 프리랜서가 700만원으로 수정
3. 클라이언트 화면
     후보 카드     700만원   ← 라이브였다
     프로필 상세   650만원   ← 얼린 값
...
**클라이언트는 카드를 보고 후보를 고른다.** 그 숫자가 협상 출발점과 달라지면 안 된다.

**조건은 얼리고 평판은 라이브로 나눴다.**

| 카드 항목 | 출처 |
|---|---|
| 직무 · 경력 · 스킬 · 단가 | 노출 시점 스냅샷 |
| 이름 · 프로필 사진 · 등급 · 평점 · 리뷰수 | 라이브 |

평판은 추천된 뒤에 쌓인 리뷰도 반영되는 게 맞다 — 사람을 고르는 데 쓰는 최신 정보다.

**"수정 후 매칭된 건은 새 값"이 이미 성립한다.** 스냅샷 키가 (프리랜서, 포지션)이라 단가를 올린 뒤
다른 포지션에 추천되면 그 포지션에는 올린 값이 얼린다. 먼저 추천된 포지션은 계속 예전 값을 본다.

**쿼리가 오히려 줄었다.** 예전에는 후보 1명당 `findCondition` 이 2쿼리(프로필→조건)씩 돌았다.
이제 포지션의 프리랜서 스냅샷을 **한 번에** 읽어(`findAllByPositionIdAndSnapshotType` 신규)
메모리에서 나눈다. 후보 10명이면 20쿼리가 1쿼리가 된다.

**폴백은 여기도 있다.** 이 코드 배포 전에 노출된 후보는 스냅샷이 없어서, 없으면 현재 조건으로 채운다.

**스킬 null 방어를 넣었다.** 스냅샷의 조건은 캡처가 부분 실패하면 스킬이 비어 있을 수 있는데,
`condition.skills().stream()` 이 그대로 NPE 를 낸다 — 카드 한 장 때문에 목록 전체가 500이 된다.

**검증** — 662 tests, 0 failures. 신규 3건.
...
## 2026-08-18 — 최초 추천·멈춘 회차 복구 완료 시 알림 누락 수정

실사용 중 발견: 클라이언트가 착수금을 결제하면 최초 추천이 비동기로 도는데(2026-08-13
"추천 준비중" 트랜잭션 분리 참고), 완료돼도 화면이 "준비중"에서 안 바뀌고 **새로고침을 해야만
후보가 보였다.**

### 원인

프론트(`RecommendedCandidates.tsx`)는 폴링을 하지 않는다. `MATCHING_RECOMMENDED` 알림을
STOMP로 받아야만 후보 목록을 다시 부른다. 그런데 이 알림은 **재추천**
(`RerecommendRequestedEventListener`, 2026-08-10부터 성공/실패 둘 다 발송)에서만 나가고
있었다. `RecruitingStartedEventListener`(최초 추천, 2026-08-09 생성)와
`StaleRoundRecoveryService`(멈춘 회차 복구, 2026-08-13 생성)는 **처음부터 알림 발송 코드
자체가 없었다** — git 히스토리 전체를 뒤져도 두 파일에 알림 관련 코드가 존재한 적이 없다.
2026-08-13 수정은 서버 쪽 상태 복구(RUNNING→preparing/FAILED, 멈춘 회차 재시도)만 다뤘고,
"복구됐다는 걸 클라이언트에게 알리는 것"은 범위에 없었다 — 같은 증상의 다른 절반이 남아있던
것이다. (알림 담당자가 최초 추천 쪽 누락을 별도로 리포트해 원인이 같음을 교차 확인함.)

### 고침

`MatchingRoundCompletionNotifier`(신규) — `RerecommendRequestedEventListener`가 쓰던
성공/실패 알림 발송 로직(`REQUIRES_NEW`, 예외 삼키고 로그만 남김)을 공용 컴포넌트로 뽑아
`RecruitingStartedEventListener.fillOrMarkFailed`와 `StaleRoundRecoveryService
.recoverStaleRounds` 양쪽에서 재사용. 클라이언트 계정 ID는 기존
`ProjectDirectoryPort.findClientAccountId(projectId)`로 구한다.

재추천 리스너는 그대로 남겨뒀다 — 실패 메시지에 "재추천 횟수는 차감되지 않습니다" 같은
재추천 전용 문구가 있어서 억지로 합치면 오히려 복잡해진다.

### 검증

`RecruitingStartedEventListenerTest`에 성공/실패 알림 테스트 2건 추가 — Mockito로 목킹하는
대신 실제 `SpringDataNotificationRepository`로 알림 행이 진짜 저장됐는지 확인한다
(`NotificationCreateUseCase`를 목으로 바꾸면 같은 클래스가 구현하는 `NotificationUseCase`까지
같이 대체돼 `NotificationController` 빈 생성이 깨진다 — 실제 DB 조회로 우회).
`StaleRoundRecoveryServiceTest`(신규, 순수 Mockito 단위테스트) 3건 — 복구 성공/재실패/대상
없음 각각의 알림 발송 여부.

`./gradlew test --tests "com.pairing.matching.*" --tests "com.pairing.freelancer.*"` 통과.
