...
| # | 검증 | 실패 시 |
|---|---|---|
| G3 | **예산 조합** — 총액 ≤ 순예산×1.2. 초과하면 제일 비싼 포지션을 다음 순위로 교체 반복 | 그래도 안 맞으면 예산 경고 후 진행 |
...
| # | 항목 | 내용 |
|---|---|---|
| U1 | **G3 예산 조합의 계산 범위** | 메모 §4는 "**프로젝트 전체 포지션**의 1순위 조합 총액"을 상정하는데, 현재 추천은 **포지션별로 따로** 돈다(`RecruitingStartedEventListener`가 포지션마다 별도 트랜잭션). 포지션 A 추천이 끝난 시점에 B는 아직 추천 전이라 전체 조합을 계산할 수 없다. ㉮ 포지션별 `budgetCap` 이내인지만 본다(현행 공식, 단순) ㉯ 전 포지션 추천 후 별도 조합 검증(구조 변경 큼) |
