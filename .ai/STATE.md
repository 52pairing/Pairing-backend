...
1. **파이프라인 (2026-08-09 수정, 2026-08-11 임베딩 대상 개정)**: 하드필터(AI매칭 동의, 직군/직무) → 임베딩 유사도(**자기소개+경력사항+조건(직무·스킬·연차·근무조건·기간)** ↔ **프로젝트설명+담당업무+업무범위+우대사항+요구스킬·최소경력·근무조건·기간**. 단가·일정은 양쪽 다 제외 — 숫자는 임베딩으로 비교가 안 되고 Stage E가 본다. 아래 "2026-08-11 갱신 — 유효 ①" 참고) → 상위 (모집인원×3) 추림 → LLM 1회 호출(포지션당, 전체 포지션 후보 한번에, 프로젝트당 1회 원칙, **일정/근무조건/단가 불일치는 여기서 감점+사유로만 반영, 후보 배제 안 함**) → 규칙기반 가드(직무/스킬 재검증 + 예산 조합 재검증)
