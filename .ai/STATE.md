...
- `preferred_note`는 포지션 전용이라 `extra_note`보다 정확하다. **지금 매칭까지 안 내려오고
  있어서 `ProjectPositionSummary`에 필드 추가 + 어댑터 매핑이 필요하다.** project 도메인 응답에
  없으면 **`extra_note`만으로 먼저 진행하고 나중에 추가한다(U7 확정)** — 3번을 기다리지 않는다.

**유사도 정규화 — 순위 기반(U3 확정, 상수 없음)**

텍스트 코사인 유사도는 0.55~0.85 같은 좁은 구간에 몰린다. 그냥 ×30 하면 "고정 보너스 16점 +
변동 9점"이 되어 **조건점수 70이 순위를 100% 결정**하고, 30:70이라는 비중이 무의미해진다.
...
| 항목 | 만점 | 공식 |
|---|---|---|
| 스킬 | 25 | `25 × 일치율 × (0.8 + 0.2 × 평균숙련도)`<br>일치율 = 보유한 요구스킬 ÷ 요구스킬 개수<br>평균숙련도 = 보유한 요구스킬들의 값 평균 (초급 0 / 중급 0.5 / 고급 1) |
| 경력연차 | 15 | 충족 → 15 / 미달 → `15 × (보유 ÷ 요구)` |
| 단가 | 15 | `budgetCap` 이내 → 15 / 초과 → `15 × (1 − 초과율)`, 최소 0<br>초과율 = (희망 − cap) ÷ cap |
| 근무방식 | 8 | 일치 또는 `ANY` → 8 / 불일치 → 0 |
| 근무형태 | 7 | 일치 또는 `ANY` → 7 / 불일치 → 0 |
| 시작일 | 5 | 협의가능 또는 시작희망일 전 → 5 / 늦음 → `5 × (1 − 지연일수/60)`, 최소 0 |
| 기간 | 5 | 협의가능(값 없음) 또는 희망 ≥ 요구 → 5 / 짧음 → `5 × (희망 ÷ 요구)` |

- **숙련도(초급/중급/고급)는 보너스이지 필수 조건이 아니다.** `position_skill`에 요구 숙련도
  컬럼이 없어서 프로젝트가 `"Java 고급 필요"`를 표현할 수 없기 때문이다.
- **연차는 초과 보유에 추가 가점을 주지 않는다.** 요구 3년에 10년차나 3년차나 15점이다.
  "3년이면 충분한데 10년차가 더 나은가"는 프로젝트 성격에 달린 판단이라 LLM(Stage E)에 맡긴다.
- 검산: `5/5 전부 초급 = 25×1.0×0.8 = 20점` **>** `3/5 전부 고급 = 25×0.6×1.0 = 15점`
  → **개수가 이긴다.** 연차 요구 3년·보유 1년 = 5점. 단가 cap 900만·희망 1,100만 = 11.7점.
...
| # | 검증 | 실패 시 |
|---|---|---|
| G3 | **예산 조합** — 노출 후보 전원의 희망 월단가 **합계** ≤ `budgetCap × 노출 인원 × 1.2` | **탈락 아님. 경고만 기록** |
...
하는데, 그건 프론트 전달 문서 갱신이 따르므로 필요해질 때 별도로 정한다(U4).
...
| U6 | 조건점수 채점식 | **확정** → [3] 표 |
| U7 | `preferred_note`가 project 응답에 없을 때 | **`extra_note`만으로 먼저 진행**하고 내려오면 추가. 3번을 기다리지 않는다 → [2] |
