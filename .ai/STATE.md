...
- **회차 레코드는 AI 호출 전에 커밋한다 (2026-08-13)**: `openInitialRound`(스냅샷+회차) 와
  ...
  없다. 재추천 경로는 원래부터 이 구조였고 최초 추천만 아니었다. **결과적으로 "회차 있음 = 후보 있음"이
  아니게 됐다** — 후보 조회는 반드시 `MatchingRoundStatus` 를 봐야 한다(`RUNNING` → `preparing`,
  `FAILED` → `failed`). 안 보면 채우는 중인 회차가 "후보 없음"으로 내려간다.
  AI 호출 중 컨테이너가 교체되면 회차가 RUNNING 으로 남으므로 `StaleRoundRecoveryService`(5분 주기)가
  다시 채운다 — 멈춘 회차엔 후보가 없어서(저장과 완료가 한 트랜잭션) 재시도가 안전하다.
...
> **이 제외 규칙 때문에 후보 목록 조회는 회차가 아니라 포지션 단위여야 한다 (2026-08-13 수정).**
> 한 번 노출된 프리랜서는 다음 회차에서 제외되므로, 화면이 최신 회차만 보여주면 이전 회차 후보는
> **다시 나올 방법이 없다.** 실제로 유료 재추천 1명을 요청한 클라이언트가 기존 2명을 잃었다.
...
> `statusLabel`로 상태를 표시한다.
>
> **이 셋은 서로 배타적이다.** 요청을 보낸 후보는 거절할 수 없고(MT_019, `rejectCandidate` 진입부),
> 거절한 후보는 요청할 수 없다(`isSelectable()` → MT_017). **양쪽을 다 막아야 배타적이다** —
> 한쪽만 막으면 두 상태가 겹치고, 그때 화면이 어느 쪽을 우선할지가 서버 판정과 갈릴 수 있다.
> 이미 보낸 요청은 프리랜서가 응답을 기다리는 중이라 카드를 내려도 사라지지 않는다는 점도 이유다.
...
## 2026-08-15 갱신 — 요청 상세에 프로젝트 본문 노출

프리랜서가 수락 전에 프로젝트를 다 보고 판단해야 한다는 프론트 요청. `GET /requests/{requestId}` 에
7개를 추가했다 — 진행 상황, 시작일 협의 가능 여부, 예상 기간(숫자+단위), 전체 모집 인원, 세부 업무
범위, 우대사항, 근무 장소.

- **상세 전용이다.** 목록/받은요청에는 안 채운다(`mainTask` 와 같은 규칙). 옛 요청이 섞인 20건
  페이지마다 프로젝트를 20번 더 읽게 되기 때문이다.
- **얼린 값 우선, 없으면 현재 값.** 기존 필드와 같은 이유로 모집 시작 시점에 얼린다(R32).
  다만 `saveSnapshotIfAbsent` 가 기존 스냅샷을 안 덮으므로 **이미 모집 중인 프로젝트는 폴백이
  없으면 영원히 안 나온다.** 옛 스냅샷 판별은 `periodValue == null`(등록 시 필수라 NOT NULL).
- **조회는 `findProjectContent(projectId)` 를 새로 만들었다.** 근무 장소·시작일 협의 여부가
  `ProjectPositionSummary` 에 없는데, 거기에 넣으면 재색인 루프·수락·재추천·회차 생성이 전부
  프로젝트를 한 번씩 더 읽는다. 화면용 필드 때문에 그럴 이유가 없다.
- **클라이언트 내부 정보는 안 담았다** — 결제·정산 상태, 재추천 사용 횟수, 확정 인원, 모집 연장
  횟수, `statusNote`. 그대로 넘겼으면 "당신은 3번째 재추천으로 뽑혔다"가 드러난다.
- **세부 업무 범위·우대사항·근무 장소는 등록 시 선택 입력이라 비어 있을 수 있다.** 프론트 빈 값
  처리 필요. 필수인 것은 `mainTask` 이고 이미 나가고 있었다.
- **첨부 자료는 못 넣었다** — 다운로드가 `hasRole('CLIENT')` 전용이라 project 도메인 결정이 필요하다.

### 같은 브랜치 — 후보 카드도 얼린 조건을 읽는다

카드만 라이브라서 화면끼리 숫자가 어긋났다(카드 700만 / 프로필 상세 650만 / 협상 시작가 650만).
클라이언트는 카드를 보고 후보를 고르므로 그 값이 협상 출발점과 같아야 한다.

- **조건은 얼린다** — 직무·경력·스킬·단가는 노출 시점 스냅샷에서 읽는다
- **평판은 라이브다** — 이름·프로필 사진·등급·평점·리뷰수. 추천된 뒤 쌓인 리뷰도 반영되는 게 맞다
- **"수정 후 매칭된 건은 새 값"은 이미 성립한다.** 스냅샷 키가 (프리랜서, 포지션)이라 단가를 올린 뒤
  다른 포지션에 추천되면 그쪽엔 올린 값이 얼린다
- **쿼리가 줄었다** — 후보 1명당 2쿼리씩 돌던 `findCondition` 이 사라지고, 포지션 스냅샷을 한 번에
  읽는다(`findAllByPositionIdAndSnapshotType` 신규). 후보 10명이면 20쿼리 → 1쿼리
- 스냅샷 없는 옛 후보는 현재 조건으로 폴백. 스냅샷 조건의 `skills` 가 null 이면 빈 목록으로 막는다
  (안 막으면 카드 한 장 때문에 목록 전체가 500)

`feat/expose-project-detail-in-matching-request` 브랜치. **662 tests, 0 failures.** 변이 3건 확인.
