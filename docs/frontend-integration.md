...
[공급자 로그인 후 프론트 콜백 라우트(/oauth/callback/kakao)에서 code, state 수신]
  POST /api/v1/auth/social/kakao/callback  { code, state }
       ...
       data.status === 'SIGNUP_REQUIRED'
         ...
         -> 추가 정보 입력(이름/전화번호/생년월일/결제수단) + 약관 동의
...
```json
{
  ...
  "paymentMethods": [
    { "methodType": "CARD", "cardNumber": "1234567812345678", "cardBrand": "신한카드" },
    { "methodType": "BANK_ACCOUNT", "bankCode": "088", "accountNo": "11012345678901", "accountHolder": "홍길동" }
  ],
  "agreements": [
...
- `paymentMethods`는 카드 1건 + 계좌 1건을 모두 보냅니다. 카드번호·계좌번호는 서버에서 암호화 저장되고, 조회 시에는 끝 4자리만 나갑니다.
...
| 카드번호 | 숫자 12~19자리 | `GLOBAL_002` |
| 계좌번호 | 숫자 6~20자리 | `GLOBAL_002` |
