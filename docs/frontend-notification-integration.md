...
| `CONTRACT_SIGNED` | 계약서 서명 | 상대방 |
| `CONTRACT_REJECTED` | 계약서 거절 | 상대방 |
...
요구사항 명세와 코드를 대조한 결과입니다. **타입은 12종 다 정의돼 있지만, 실제로
발행되는 건 7종입니다.**
...
| `CONTRACT_SIGNED` | 클라 6 / 프리 5 — 계약서 수락 |
...
| `CONTRACT_REJECTED` | 클라 6 / 프리 5 — 계약서 **거절** | 계약 |

알림 저장·조회·읽음·삭제·실시간 push 는 전부 준비돼 있고, **"언제 알림을 만들지"를
각 도메인이 호출**해야 하는 구조입니다. 협상 도메인과 계약 거절 경로에서 호출이 빠져 있어
각 담당께 요청해둔 상태입니다.

**프론트는 이 4종도 온다고 가정하고 만드셔도 됩니다.** 서버 쪽 호출만 붙으면
