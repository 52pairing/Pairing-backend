...
| 역할 | 탈퇴를 막지 않는 상태 |
|---|---|
| 클라이언트 | 프로젝트가 **등록 완료 · 종료 · 취소됨** |
| 프리랜서 | 계약이 **종료 · 중도 종료 · 서명 거부** |
| 공통 | **미납 수수료가 없을 것** |

> **`등록 완료`(REGISTERED)는 탈퇴를 막지 않습니다.** (2026-08-13 수정)
> 아직 모집도 시작하지 않아 기다리는 상대가 없습니다. 프로젝트만 올려두고 쓰지 않은
> 사용자가 탈퇴하지 못하던 문제를 고쳤습니다.
