...
| errorCode | 뜻 | 쿠키 | 프론트 대응 |
| --- | --- | --- | --- |
| `GLOBAL_006` | 토큰 없음 (비로그인) | 해당 없음 | 로그인 페이지로 |
| `GLOBAL_009` | Access 만료 | 유지 | **`POST /auth/refresh` 후 원래 요청 1회 재시도** |
| `GLOBAL_010` | 토큰 위조·손상 | **서버가 만료시킴** | 로그인 페이지로 |
| `GLOBAL_011` | **다른 기기에서 로그인해 세션이 끊김** | **서버가 만료시킴** | "다른 기기에서 로그인되었습니다" 모달 → 로그인 페이지로 |

> **쿠키 정리는 프론트가 하지 않습니다.** 세션이 끊긴 401 응답에는 서버가 `Set-Cookie ... Max-Age=0` 을 함께 실어 보냅니다.
> `accessToken`/`refreshToken` 은 HttpOnly 라 JS 로 지울 수 없고, 서버가 지워 주지 않으면 죽은 토큰이 남아 로그인 화면에서까지
> 같은 401 이 반복됩니다. 자세한 계약은 [frontend-session-cookie-expiry.md](frontend-session-cookie-expiry.md) 를 보세요.
...
`refresh` 자체가 실패했을 때는 두 코드를 갈라야 합니다.

| 코드 | 뜻 | 쿠키 | 대응 |
| --- | --- | --- | --- |
| `AU_015` | 다른 기기 로그인으로 세션 종료 | **서버가 만료시킴** | "다른 기기에서 로그인" 모달 → 로그인 페이지 |
| `AU_016` | 리프레시 토큰 무효 **또는 같은 세션의 중복 재발급** | 유지 | 원래 요청을 **1회만** 재시도. 그래도 실패하면 로그인 페이지 |

`AU_016` 에 중복 재발급이 포함됩니다. 탭을 두 개 열어두면 둘이 동시에 재발급을 요청하는 일이 생기는데, 밀린 쪽은
브라우저에 이미 갱신된 유효한 쿠키가 있으므로 한 번 더 보내면 성공합니다. 여기서 모달을 띄우면 멀쩡히 로그인된 사용자를
로그인 화면으로 보내게 됩니다. 재시도는 반드시 1회로 제한하세요 — 토큰이 정말 죽은 경우에도 같은 코드가 옵니다.
자세한 배경은 [frontend-session-cookie-expiry.md](frontend-session-cookie-expiry.md) 3절에 있습니다.
...
| 새 비밀번호 등록 (임시 비밀번호 로그인 직후) | `PATCH /auth/password` **한 번만** (인증코드 불필요) |
...
| `AU_014` | 429 | IP 차단. 1시간에 20회 실패하면 2시간 차단. **`message` 에 재시도 가능 시각이 들어 있으니 그대로 보여주면 된다** |
...
`message` 에 **언제부터 다시 되는지**가 들어 있습니다. 별도로 계산하지 말고 그대로 노출하세요.

```jsonc
// 남은 시간을 읽은 경우
{ "status": 429, "errorCode": "AU_014",
  "message": "로그인 시도가 많아 접근이 제한되었습니다. 2026-08-07 18:30 이후에 다시 시도해 주세요. (약 1시간 30분 남음)" }

// 남은 시간을 못 읽은 경우 (Redis 키가 막 만료되는 등)
{ "status": 429, "errorCode": "AU_014",
  "message": "로그인 시도가 많아 접근이 제한되었습니다. 2시간 후에 다시 시도해 주세요." }
```

`errorCode` 는 두 경우 모두 `AU_014` 라 분기 로직은 그대로 두면 됩니다.
...
메일로 받은 임시 비밀번호로 로그인하면 `tempPassword: true` 가 옵니다. **새 비밀번호 등록 화면으로 강제 이동**시켜야 합니다.
(5-1 의 주의 참고 — 서버가 막아 주지 않으므로 프론트 라우터 가드로 처리합니다.)

이 화면은 입력칸이 **새 비밀번호 / 새 비밀번호 확인 둘뿐**입니다. 인증코드를 다시 받지 않습니다.
...
> 8절(마이페이지 변경)과 **같은 엔드포인트지만 인증코드가 필요 없습니다.**
> 재설정 링크를 열고 임시 비밀번호 메일까지 받아 로그인한 상태라 메일 소유가 이미 두 번 증명됐기 때문입니다.
> 서버가 `tempPassword` 상태를 보고 인증 검사를 건너뜁니다.

- 임시 비밀번호와 같은 값을 넣으면 `AU_028` 입니다.
- 성공하면 세션이 끊기므로 로그인 화면으로 보냅니다.
- 화면 문구: "보안을 위해 새로운 비밀번호를 등록해야 합니다. 등록 전까지 다른 서비스에 접근할 수 없습니다."
  이 차단은 현재 **프론트 라우터 가드 책임**입니다.
...
> `PATCH /auth/password` 는 두 화면이 함께 씁니다. **인증코드 요구만 다릅니다.**
>
> | 호출 화면 | 인증코드 | 이유 |
> | --- | --- | --- |
> | 마이페이지 비밀번호 변경 | **필요** | 세션만으로 비밀번호를 바꾸면 세션 탈취 시 계정을 뺏긴다 |
> | 임시 비밀번호 로그인 직후 | 불필요 | 링크 + 임시 비밀번호 메일로 메일 소유를 이미 증명했다 |
>
> 서버가 계정의 `tempPassword` 상태를 보고 알아서 갈라 줍니다. 프론트는 같은 API 를 호출하면 됩니다.
...
| `GLOBAL_011` | 401 | 다른 기기 로그인으로 세션 종료 (쿠키도 함께 만료됨) |
...
| `AU_015` | 401 | 재발급 중 다른 기기 로그인 감지 (쿠키도 함께 만료됨) |
| `AU_016` | 401 | 리프레시 토큰 없음·무효, 또는 같은 세션의 중복 재발급 (쿠키 유지) |
...
- [ ] 임시 비밀번호 로그인 직후 화면은 인증코드 없이 `PATCH /auth/password` 만 호출한다
