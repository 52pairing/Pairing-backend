...
| --- | --- | --- |
| 프론트 주소 | `APP_FRONT_BASE_URL` (기본 `http://localhost:3000`) | 비밀번호 재설정 메일 링크가 이 주소로 만들어진다. 실제 프론트 주소와 같아야 한다 |
| 허용 오리진 | `CORS_ALLOWED_ORIGINS` (기본 `http://localhost:3000,http://127.0.0.1:3000`) | 프론트를 띄운 오리진이 목록에 있어야 한다. `localhost` 와 `127.0.0.1` 은 다른 오리진이다 |
...
│   http://localhost:3000/reset-password?token=9f2c1e5a-....           │
└──────────────────────────────────────────────────────────────────────┘
                              │  클릭
...
- `APP_FRONT_BASE_URL` 은 **서버 환경변수**입니다. 프론트가 정하는 값이 아닙니다.
  로컬 기본값이 `http://localhost:3000` 이라, 프론트를 3000 이 아닌 포트로 띄우면 링크가 깨집니다.
