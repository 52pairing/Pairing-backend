...
| 3 | [결제수단 수정 시 이메일 인증](#3-결제수단을-바꿀-때-이메일-인증이-생깁니다) | **필수** | 💥 안 고치면 결제수단 수정 400 |
...
## 3. 결제수단을 바꿀 때 이메일 인증이 생깁니다

**수정할 때만** 인증이 필요합니다. 조회는 그대로입니다. 계정을 잠깐 빌린 사람이 정산 계좌를
자기 것으로 바꿔치기하는 걸 막는 게 목적입니다.
...
처음에는 조회까지 막았습니다. 마스킹해도 은행명·예금주·끝 4자리가 단서가 된다는 이유였습니다.
그런데 그러면 **수수료 결제가 함께 막힙니다.** 결제 모달도 결제할 카드를 고르려고 같은 API 를
부르기 때문에, 결제하려는 사람이 매번 이메일 인증을 거쳐야 했습니다.

막아야 할 위험은 계정을 잠깐 빌린 사람이 **정산 계좌를 자기 것으로 바꿔치기**하는 것이고,
그건 수정만 막으면 됩니다. 그래서 **PUT 두 건에만** 관문을 남겼습니다.

덕분에 프론트에서 **수수료 결제 모달은 고칠 게 없습니다.** 마이페이지 결제수단 탭의 수정 흐름만
손보면 됩니다.

계약서의 정산 계좌 표기, 정산 목록의 `"신한카드 **** 1234"` 표기도 **인증 없이 그대로** 동작합니다.
...
| `AU_006` | 400 | **이메일 인증 미완료** (결제수단 수정 포함) |
