...
public interface DepositSettlementUseCase {
     ...
     *
     * <p><b>클라이언트 착수금 1건만</b> 본다. 등록 취소는 착수금 결제 전에만 가능해
     * 그때는 프로젝트에 걸린 정산이 그것뿐이다.
     */
    void cancelPayable(Long projectId);

    /**
     * 프로젝트가 취소돼 낼 이유가 사라진 정산을 <b>전부</b> 취소한다. (정책 P46)
     *
     * <p>{@link #cancelPayable} 과 나눠 둔다. 그쪽은 등록 취소용이라 클라이언트 착수금 1건만
     * 보는데, 모집 기간 만료 시점에는 계약이 체결된 만큼 <b>프리랜서 착수금이 여러 건</b> 붙어
     * 있다. 1건만 지우면 나머지가 남아 프리랜서 화면에 "결제 필요" 로 계속 뜬다.
     *
     * <p>이미 결제된 건은 건드리지 않는다. 낸 돈을 되돌리는 건 환불이라 별개이고, 환불 처리는
     * 아직 없다. 화면이 프로젝트 취소 여부를 보고 "환불 대상" 으로 안내한다.
     *
     * @return 취소한 건수
     */
    int cancelAllPayable(Long projectId);
}
